...
const CONFIG = {
  ...
  AREA_MODULES: {
    ...
    CULTOS: ['relatorio_cultos','pequenos_grupos'],
...
function normalizeAndValidateSubmissionV541_(payload, sessionUser) {
  ...
  if (module === 'pequenos_grupos') normalizePequenosGruposV547_(out.data, out.repeats);
...
function normalizePequenosGruposV547_(data, repeats) {
  const required = {
    pg_grupo: 'Indique o grupo.',
    pg_local: 'Indique o local do encontro.',
    pg_data: 'Indique a data do encontro.',
    pg_hora: 'Indique a hora do encontro.'
  };
  Object.keys(required).forEach(function(key) {
    if (!String(data[key] == null ? '' : data[key]).trim()) throw new Error(required[key]);
  });

  const presentes = Number(data.pg_membros_presentes);
  if (!isFinite(presentes) || presentes < 0 || Math.floor(presentes) !== presentes) {
    throw new Error('O número de membros presentes deve ser um número inteiro igual ou superior a zero.');
  }
  data.pg_membros_presentes = presentes;
  data.pg_novos_participantes = normalizeSimNaoV547_(data.pg_novos_participantes);
  data.pg_fizeram_ofertas = normalizeSimNaoV547_(data.pg_fizeram_ofertas);

  const participantes = Array.isArray(repeats.pg_actualizacao_membros) ? repeats.pg_actualizacao_membros : [];
  const categorias = { membro_nao_registado:true, novo_membro:true, visitante:true, crianca:true };
  if (data.pg_novos_participantes === 'sim') {
    if (!participantes.length) throw new Error('Adicione pelo menos um novo participante.');
    participantes.forEach(function(row) {
      if (!String(row.pg_participante_nome || '').trim()) throw new Error('Indique o nome de cada novo participante.');
      if (!categorias[String(row.pg_participante_categoria || '').trim()]) throw new Error('Seleccione uma categoria válida para cada novo participante.');
    });
  } else {
    repeats.pg_actualizacao_membros = [];
  }
  data.pg_novos_participantes_qtd = data.pg_novos_participantes === 'sim' ? participantes.length : 0;

  const fields = ['pg_ofertas','pg_construcao','pg_outras_ofertas'];
  let total = 0;
  fields.forEach(function(key) {
    const value = data.pg_fizeram_ofertas === 'sim' ? parseMoney_(data[key]) : 0;
    if (!isFinite(value) || value < 0) throw new Error('As ofertas e contribuições devem ter valores iguais ou superiores a zero.');
    data[key] = value;
    total += value;
  });
  data.pg_total_ofertas = total;
}

function normalizeSimNaoV547_(value) {
  const s = String(value || '').trim().toLowerCase();
  if (['sim','s','yes','true','1'].indexOf(s) >= 0) return 'sim';
  if (['nao','não','n','no','false','0'].indexOf(s) >= 0) return 'nao';
  throw new Error('Seleccione Sim ou Não.');
}
...
function approvalSummary_(obj, module) {
  ...
  if (module === 'pequenos_grupos') {
    return [obj.pg_data || obj.data, obj.pg_grupo || obj.grupo, obj.pg_membros_presentes !== '' ? 'Presentes: ' + obj.pg_membros_presentes : '', obj.pg_total_ofertas !== '' ? 'Ofertas: ' + obj.pg_total_ofertas + ' MT' : ''].filter(Boolean).join(' — ');
  }
...
function getAppData_(user, params) {
  ...
  if (canViewAreaForData_(user, 'CULTOS')) {
    data.cultos = getCultosData_(user, scopeId);
    data.pequenosGrupos = getPequenosGruposData_(user, scopeId);
  }
...
function getPequenosGruposData_(user, requestedChurchId) {
  return filterObjectsByChurch_(readSheetObjects_(CONFIG.MODULE_SHEETS.pequenos_grupos), user, requestedChurchId)
    .filter(isUsableRow_)
    .map(function(obj) {
      return {
        data: dateIso_(rowValue_(obj, ['pg_data','data','submittedAt','createdAt'])),
        hora: timeText_(rowValue_(obj, ['pg_hora','hora'])),
        grupo: rowValue_(obj, ['pg_grupo','grupo']),
        local: rowValue_(obj, ['pg_local','local']),
        membrosPresentes: parseMoney_(rowValue_(obj, ['pg_membros_presentes','membros_presentes','presentes'])),
        novosParticipantes: parseMoney_(rowValue_(obj, ['pg_novos_participantes_qtd','novos_participantes_qtd'])),
        ofertas: parseMoney_(rowValue_(obj, ['pg_total_ofertas','total_ofertas','ofertas'])),
        observacoes: rowValue_(obj, ['pg_observacoes','observacoes','comentarios'])
      };
    })

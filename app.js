...
async function handleLoginSubmit(e){
  ...
  try{
    ...
    // Carrega dados do sistema sem voltar automaticamente ao ecrã de login caso algum módulo falhe.
    try {
      await bootstrap();
    } catch(bootErr) {
      console.warn('Bootstrap falhou depois do login, mas a sessão foi mantida:', bootErr);
      setStatus('bad','Dados não carregados','Login efectuado; verifique o backend para carregar dados.');
    }
    return false;
  }catch(err){

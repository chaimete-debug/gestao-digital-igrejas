...
const state = {
  ...
  avante: { data:null, selectedGroup:'', loading:false, comparisonBaseYear:null, comparisonYear:null, trendChurchId:'' },
  approvals: { items: [], selected: null, loading: false },
  userAdmin: { users: [], profiles: [], churches: [], departments: [], audit: [], loading: false }
};
...
function updateInstallAppUi(){
  ...
  if(isInstalledAppMode()){
    btn.textContent = 'Aplicação instalada';
    btn.disabled = true;
    hint.textContent = 'A aplicação já está disponível no ecrã principal deste telefone.';
    return;
  }
  btn.disabled = false;
  btn.textContent = 'Instalar aplicação';
  if(isAppleMobileDevice()){
    hint.textContent = 'No Safari, toque em Partilhar e depois em “Adicionar ao ecrã principal”.';
  }else if(deferredInstallPrompt){
    hint.textContent = 'Toque no botão para criar o ícone da aplicação no telefone.';
  }else{
...
async function requestInstallApp(){
  ...
  if(deferredInstallPrompt){
    ...
    promptEvent.prompt();
    try { await promptEvent.userChoice; } catch(_err) {}
    updateInstallAppUi();
    return;
  }
  if(isAppleMobileDevice()){
...
// v54.5.0: mantém a autenticação por separador e aplica escopo de leitura por igreja. Ao fechar o
...
async function setSelectedWorkChurch(id){
  ...
  // v54.5.0: a alteração da igreja actualiza imediatamente todas as
...
function updateChurchScopeUi(){
  ...
  const topChurchLabel=$('#topWorkChurchLabel');
  const topChurchName=$('#topWorkChurchName');

  const visibleChurchName = district
    ? (church ? church.name : 'Visão distrital — todas as igrejas')
  ...
  if(selectedNameEl){
    selectedNameEl.textContent=church
      ? church.name
      : (district ? 'Visão distrital — todas as igrejas (apenas consulta).' : 'Nenhuma igreja seleccionada.');
    selectedNameEl.title=church ? church.name : '';
    selectedNameEl.classList.toggle('empty',district && !church);
  }

  if(topChurchBar && topChurchName){
    const hasChurch=!!church;
    if(topChurchLabel) topChurchLabel.textContent=hasChurch ? 'Igreja de trabalho' : (district ? 'Escopo de consulta' : 'Igreja');
    topChurchName.textContent=hasChurch
      ? church.name
      : (district ? 'Visão distrital — todas as igrejas' : 'Igreja não seleccionada');
    topChurchName.title=hasChurch
      ? church.name
      : (district ? 'Os módulos de consulta apresentam dados consolidados do Distrito. Para criar registos, seleccione uma igreja.' : 'Igreja não seleccionada');
    topChurchBar.classList.toggle('empty',!hasChurch && !district);
    topChurchBar.classList.toggle('district-view',district && !hasChurch);
    topChurchBar.setAttribute('aria-label',hasChurch
      ? `Igreja de trabalho: ${church.name}`
      : (district ? 'Visão distrital — todas as igrejas' : 'Igreja de trabalho não seleccionada'));
  }
  ...
  if(guard){
    if(district && !church){
      ...
      guard.textContent='Está na Visão Distrital. Seleccione a Igreja de trabalho no painel lateral antes de preencher ou submeter um novo registo.';
    }else{
...
function currentReadScopeParams_(){
  const church=getEffectiveWorkChurch();
  return church && church.id ? {igreja_id:String(church.id)} : {};
}

async function refreshScopedViewsAfterChurchChange_(){
  populateApprovalFilters();
  const selected=getSelectedWorkChurchId();
  const approvalChurch=$('#approvalChurchFilter');
  if(approvalChurch && isDistrictUserClient()){
    approvalChurch.value=selected || '';
  }

  const jobs=[loadStats(),loadAppData()];
  if(state.currentView==='reportView') jobs.push(loadFinancialReport());
  if(state.currentView==='approvalView') jobs.push(loadApprovals());
...
function userCanManageSystemConfig(){
  const u = currentUser();
  const role = String(u.role || u.perfil || '').toUpperCase();
  return !!u.canConfig || role.includes('ADMIN');
}
...
function applyPermissionsToUi(){
  ...
  $$('.admin-config-only').forEach(el => el.classList.toggle('hidden', !userCanManageSystemConfig()));
...
async function handleLoginSubmit(e){
  ...
  try{
    ...
    showApp(user);
    injectMultiSelectStyles();
    if(userMustChangePassword(user) || out.mustChangePassword){
      setStatus('bad','Alteração obrigatória','Crie uma nova palavra-passe para continuar');
      openPasswordDialog(true);
      toast('Deve alterar a palavra-passe temporária antes de continuar.','info');
      return false;
    }
...
function escapeHtmlV5441(value){
  return String(value == null ? '' : value).replace(/[&<>"']/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[ch]));
}

function userMustChangePassword(user){
  return !!(user && (user.mustChangePassword === true || String(user.mustChangePassword || '').toUpperCase() === 'SIM'));
}
...
function openPasswordDialog(force=false){
  ...
  $('#passwordDialogTitle').textContent=force?'Alteração obrigatória da palavra-passe':'Alterar palavra-passe';
  $('#passwordDialogMessage').textContent=force
    ? 'A palavra-passe actual é temporária. Deve criar uma nova palavra-passe antes de utilizar o sistema.'
    : 'Use uma palavra-passe com pelo menos 8 caracteres, incluindo maiúscula, minúscula e número.';
  $('#passwordCancelBtn')?.classList.toggle('hidden',force);
  $('#passwordDialogClose')?.classList.toggle('hidden',force);
  ['#currentPasswordInput','#newPasswordInput','#confirmPasswordInput'].forEach(sel=>{const el=$(sel);if(el)el.value='';});
  const result=$('#passwordChangeResult'); if(result){result.className='member-import-result hidden';result.textContent='';}
...
function closePasswordDialog(){
  const dialog=$('#passwordDialog');
  if(!dialog || dialog.dataset.force==='1') return;
  dialog.close();
}
...
async function submitOwnPasswordChange(event){
  ...
  const button=$('#saveOwnPasswordBtn');
  if(result){result.className='member-import-result hidden';result.textContent='';}
  if(!currentPassword||!newPassword||!confirmPassword){toast('Preencha todos os campos da palavra-passe.','error');return;}
  if(newPassword!==confirmPassword){toast('A confirmação da nova palavra-passe não coincide.','error');return;}
  if(SANDBOX_MODE){toast('Modo teste: palavra-passe não alterada.');return;}
  try{
    if(button){button.disabled=true;button.textContent='A guardar...';}
    const res=await fetch(window.APP_CONFIG.APPS_SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'changeOwnPassword',authToken:getAuthToken(),currentPassword,newPassword,confirmPassword})});
    const out=await res.json();
    if(!out.ok) throw new Error(out.message||'Não foi possível alterar a palavra-passe.');
    saveAuthSession(out.token,out.user);
    state.currentUser=out.user;
    state.members=out.members||[];
    state.churches=out.churches||[];
    state.memberScope=out.memberScope||state.memberScope;
    const dialog=$('#passwordDialog'); if(dialog?.open) dialog.close();
    setCurrentUserCard(out.user);
    applyPermissionsToUi();
    renderForm();
    ensureMembersForCurrentChurch();
    loadStats(); loadAppData();
    setStatus('ok','Ligado','Palavra-passe alterada e sessão renovada');
    window.clearTimeout(state.passwordStatusTimer);
    state.passwordStatusTimer = window.setTimeout(()=>{
      if(currentUser()) setStatus('ok','Ligado','Sessão activa');
    }, 5000);
    toast('Palavra-passe alterada com sucesso.','success');
  }catch(err){
    const msg=String(err?.message||'Falha ao alterar a palavra-passe.');
    if(result){result.className='member-import-result error';result.textContent=msg;}
    toast(msg,'error');
  }finally{if(button){button.disabled=false;button.textContent='Guardar nova palavra-passe';}}
}

function formatUserDateV5441(value){
  if(!value)return '—';
  const d=new Date(value);if(Number.isNaN(d.getTime()))return escapeHtmlV5441(value);
  return d.toLocaleString('pt-MZ',{dateStyle:'short',timeStyle:'short'});
}

function profileLabelV5441(role){
  const item=(state.userAdmin.profiles||[]).find(p=>p.value===role);
  return item?.label||role||'—';
}

function userStatusBadgesV5441(user){
  const badges=[];
  badges.push(`<span class="status-pill ${user.active?'active':'inactive'}">${user.active?'Activo':'Inactivo'}</span>`);
  if(user.isBlocked)badges.push('<span class="status-pill blocked">Bloqueado</span>');
  if(user.mustChangePassword)badges.push('<span class="status-pill warning">Troca pendente</span>');
  return badges.join(' ');
}

function filteredAdminUsersV5441(){
  const q=String($('#usersAdminSearch')?.value||'').trim().toLowerCase();
  const role=$('#usersAdminRoleFilter')?.value||'';
  const status=$('#usersAdminStatusFilter')?.value||'';
  return (state.userAdmin.users||[]).filter(u=>{
    const hay=[u.name,u.username,u.igreja_nome,u.department].join(' ').toLowerCase();
    ...
    if(status==='CHANGE'&&!u.mustChangePassword)return false;
    return true;
  });
}

function renderUsersAdmin(){
  const users=filteredAdminUsersV5441();
  const all=state.userAdmin.users||[];
  const active=all.filter(u=>u.active).length;
  const local=all.filter(u=>u.accessLevel==='LOCAL').length;
  const blocked=all.filter(u=>u.isBlocked).length;
  const pending=all.filter(u=>u.mustChangePassword).length;
  const kpis=$('#usersAdminKpis');
  if(kpis)kpis.innerHTML=`<div class="legacy-kpi"><span>Utilizadores</span><strong>${all.length}</strong></div><div class="legacy-kpi"><span>Activos</span><strong>${active}</strong></div><div class="legacy-kpi"><span>Contas locais</span><strong>${local}</strong></div><div class="legacy-kpi"><span>Bloqueados</span><strong>${blocked}</strong></div><div class="legacy-kpi"><span>Troca pendente</span><strong>${pending}</strong></div>`;
  if($('#usersAdminCount'))$('#usersAdminCount').textContent=`${users.length} de ${all.length} utilizador(es)`;
  const tbody=$('#usersAdminTable tbody');
  if(tbody)tbody.innerHTML=users.length?users.map(u=>`<tr>
    <td><strong>${escapeHtmlV5441(u.name)}</strong><small class="table-subline">${escapeHtmlV5441(u.username)}</small></td>
    <td>${escapeHtmlV5441(profileLabelV5441(u.role))}</td>
    <td><strong>${escapeHtmlV5441(u.accessLevel==='DISTRITAL'?'Distrital':'Local')}</strong><small class="table-subline">${escapeHtmlV5441(u.igreja_nome||'Todas as igrejas')}</small></td>
    <td>${escapeHtmlV5441(u.department||'—')}</td>
    <td>${userStatusBadgesV5441(u)}</td>
    <td>${formatUserDateV5441(u.lastLogin)}</td>
    <td class="user-action-cell"><button type="button" class="mini-btn" data-user-edit="${escapeHtmlV5441(u.id)}">Editar</button><button type="button" class="mini-btn reset" data-user-reset="${escapeHtmlV5441(u.id)}">Redefinir senha</button></td>
  </tr>`).join(''):'<tr><td colspan="7">Nenhum utilizador corresponde aos filtros.</td></tr>';

  const auditBody=$('#usersAuditTable tbody');
  const audit=state.userAdmin.audit||[];
  if(auditBody)auditBody.innerHTML=audit.length?audit.map(a=>`<tr><td>${formatUserDateV5441(a.createdAt)}</td><td>${escapeHtmlV5441(a.actor)}</td><td>${escapeHtmlV5441(a.action)}</td><td>${escapeHtmlV5441(a.details)}</td></tr>`).join(''):'<tr><td colspan="4">Sem histórico administrativo.</td></tr>';
}

function populateUserAdminSelectors(){
  const role=$('#adminUserRole');
  if(role)role.innerHTML=(state.userAdmin.profiles||[]).map(p=>`<option value="${escapeHtmlV5441(p.value)}">${escapeHtmlV5441(p.label)}</option>`).join('');
  const filter=$('#usersAdminRoleFilter');
  if(filter){const current=filter.value;filter.innerHTML='<option value="">Todos os perfis</option>'+(state.userAdmin.profiles||[]).map(p=>`<option value="${escapeHtmlV5441(p.value)}">${escapeHtmlV5441(p.label)}</option>`).join('');filter.value=current;}
  const church=$('#adminUserChurch');
  if(church)church.innerHTML='<option value="">Seleccione...</option>'+(state.userAdmin.churches||[]).map(c=>`<option value="${escapeHtmlV5441(c.igreja_id||c.id)}">${escapeHtmlV5441(c.nome_igreja||c.name)}</option>`).join('');
  const dep=$('#adminUserDepartment');
  if(dep)dep.innerHTML=(state.userAdmin.departments||[]).map(d=>`<option value="${escapeHtmlV5441(d)}">${escapeHtmlV5441(d==='*'?'Sem departamento específico':d)}</option>`).join('');
}

async function loadUsersAdmin(){
  if(SANDBOX_MODE)return;
  if(!currentUser()?.canConfig&&!String(currentUser()?.role||'').includes('ADMIN'))return;
  const tbody=$('#usersAdminTable tbody');if(tbody)tbody.innerHTML='<tr><td colspan="7">A carregar utilizadores...</td></tr>';
  try{
    const params=new URLSearchParams({action:'usersadmin',token:getAuthToken()});
    const res=await fetch(window.APP_CONFIG.APPS_SCRIPT_URL+'?'+params.toString(),{cache:'no-store'});
    const out=await res.json();if(!out.ok)throw new Error(out.message||'Não foi possível carregar os utilizadores.');
    state.userAdmin={...state.userAdmin,...out,loading:false};
    populateUserAdminSelectors();renderUsersAdmin();
  }catch(err){console.error(err);if(tbody)tbody.innerHTML=`<tr><td colspan="7">${escapeHtmlV5441(err.message)}</td></tr>`;toast(err.message,'error');}
}

function updateAdminUserFormVisibility(){
  const district=$('#adminAccessLevel')?.value==='DISTRITAL';
  $('#adminChurchField')?.classList.toggle('hidden',district);
  if(district&&$('#adminUserChurch'))$('#adminUserChurch').value='';
  const leader=$('#adminUserRole')?.value==='LIDER';
  $('#adminUserDepartment')?.closest('label')?.classList.toggle('field-attention',leader);
}

function openUserAdminDialog(user=null){
  populateUserAdminSelectors();
  const dialog=$('#userAdminDialog');if(!dialog)return;
  $('#userAdminDialogTitle').textContent=user?'Editar utilizador':'Novo utilizador';
  $('#adminUserId').value=user?.id||'';$('#adminOriginalUsername').value=user?.username||'';
  $('#adminUserName').value=user?.name||'';$('#adminUsername').value=user?.username||'';
  $('#adminUserRole').value=user?.role||'VISUALIZADOR';$('#adminAccessLevel').value=user?.accessLevel||'LOCAL';
  $('#adminUserChurch').value=user?.igreja_id==='*'?'':(user?.igreja_id||'');$('#adminUserDepartment').value=user?.department||'*';
  $('#adminUserActive').checked=user?!!user.active:true;$('#adminInitialPassword').value='';
  $('#adminInitialPasswordField')?.classList.toggle('hidden',!!user);$('#adminInitialPassword').required=!user;
  const result=$('#userAdminFormResult');if(result){result.className='member-import-result hidden';result.textContent='';}
  updateAdminUserFormVisibility();dialog.showModal();
}

function closeUserAdminDialog(){const d=$('#userAdminDialog');if(d?.open)d.close();}

async function saveUserAdmin(event){
  event?.preventDefault();
  const id=$('#adminUserId')?.value||'';
  const data={id,originalUsername:$('#adminOriginalUsername')?.value||'',name:$('#adminUserName')?.value||'',username:$('#adminUsername')?.value||'',role:$('#adminUserRole')?.value||'',accessLevel:$('#adminAccessLevel')?.value||'',igreja_id:$('#adminUserChurch')?.value||'',department:$('#adminUserDepartment')?.value||'*',active:!!$('#adminUserActive')?.checked,initialPassword:$('#adminInitialPassword')?.value||''};
  const button=$('#saveUserAdminBtn'),result=$('#userAdminFormResult');
  try{
    if(button){button.disabled=true;button.textContent='A guardar...';}
    const res=await fetch(window.APP_CONFIG.APPS_SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'saveUserAdmin',authToken:getAuthToken(),data})});
    const out=await res.json();if(!out.ok)throw new Error(out.message||'Não foi possível guardar o utilizador.');
    state.userAdmin={...state.userAdmin,...out};populateUserAdminSelectors();renderUsersAdmin();closeUserAdminDialog();toast(id?'Utilizador actualizado.':'Utilizador criado.','success');
  }catch(err){if(result){result.className='member-import-result error';result.textContent=err.message;}toast(err.message,'error');}
  finally{if(button){button.disabled=false;button.textContent='Guardar utilizador';}}
}

async function resetAdminUserPassword(userId){
  const user=(state.userAdmin.users||[]).find(u=>u.id===userId);if(!user)return;
  const newPassword=window.prompt(`Introduza uma palavra-passe temporária para ${user.name}.\nDeve ter pelo menos 8 caracteres, maiúscula, minúscula e número.`,'');
  if(newPassword===null)return;
  if(!window.confirm(`Confirma a redefinição da palavra-passe de ${user.name}?`))return;
  try{
    const res=await fetch(window.APP_CONFIG.APPS_SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'resetUserPassword',authToken:getAuthToken(),userId,newPassword})});
    const out=await res.json();if(!out.ok)throw new Error(out.message||'Não foi possível redefinir a palavra-passe.');
    toast(out.message||'Palavra-passe redefinida.','success');await loadUsersAdmin();
  }catch(err){toast(err.message,'error');}
}

function bindSecurityAndUserAdmin(){
  $('#configChangePasswordBtn')?.addEventListener('click',()=>openPasswordDialog(false));
  $('#changePasswordBtn')?.addEventListener('click',()=>openPasswordDialog(false));
  $('#passwordForm')?.addEventListener('submit',submitOwnPasswordChange);
  ...
  $('#passwordDialog')?.addEventListener('cancel',e=>{if(e.currentTarget.dataset.force==='1')e.preventDefault();});
  $('#refreshUsersAdminBtn')?.addEventListener('click',loadUsersAdmin);
  $('#newUserAdminBtn')?.addEventListener('click',()=>openUserAdminDialog(null));
  $('#userAdminForm')?.addEventListener('submit',saveUserAdmin);
  $('#userAdminDialogClose')?.addEventListener('click',closeUserAdminDialog);
  $('#cancelUserAdminBtn')?.addEventListener('click',closeUserAdminDialog);
  $('#adminAccessLevel')?.addEventListener('change',updateAdminUserFormVisibility);
  $('#adminUserRole')?.addEventListener('change',updateAdminUserFormVisibility);
  ['#usersAdminSearch','#usersAdminRoleFilter','#usersAdminStatusFilter'].forEach(sel=>$(sel)?.addEventListener(sel.includes('Search')?'input':'change',renderUsersAdmin));
  $('#usersAdminTable')?.addEventListener('click',e=>{
    const edit=e.target.closest('[data-user-edit]');if(edit){const u=(state.userAdmin.users||[]).find(x=>x.id===edit.dataset.userEdit);if(u)openUserAdminDialog(u);return;}
    const reset=e.target.closest('[data-user-reset]');if(reset)resetAdminUserPassword(reset.dataset.userReset);
  });
}
...
async function bootstrap(){
  ...
  try{
    ...
    if(userMustChangePassword(state.currentUser) || out.mustChangePassword){
      setStatus('bad','Alteração obrigatória','Crie uma nova palavra-passe para continuar');
      openPasswordDialog(true);
      return;
    }
...
async function loadStats(){
  ...
  const url=window.APP_CONFIG.APPS_SCRIPT_URL;
  if(!url){ grid.innerHTML='<div class="kpi"><span>Backend não configurado</span></div>'; return; }
  try{
    const token=getAuthToken();
    if(!token){ showLogin('Sessão expirada. Faça login novamente.'); return; }
    const params=new URLSearchParams({action:'stats',token,...currentReadScopeParams_()});
    ...
    console.error(e);
    grid.innerHTML='<div class="kpi"><span>Não foi possível carregar o painel.</span></div>';
  }
}


function renderRecent(rows,scope){
  const thead=$('#recentTable thead'), tbody=$('#recentTable tbody');
  if(!thead || !tbody) return;
  const showChurch=!!(scope && scope.districtView);
  thead.innerHTML=showChurch
    ? '<tr><th>Data</th><th>Igreja</th><th>Módulo</th><th>Estado</th><th>UUID</th></tr>'
    : '<tr><th>Data</th><th>Módulo</th><th>Estado</th><th>UUID</th></tr>';
  tbody.innerHTML=rows.length ? rows.map(r=>showChurch
    ? `<tr><td>${r.submittedAt||''}</td><td>${cleanHtml(r.igreja_nome||r.igreja_id||'Sem igreja')}</td><td>${optionLabel('menu_preencher',r.module)||r.module}</td><td>${cleanHtml(r.estado_aprovacao||'')}</td><td>${r.uuid||''}</td></tr>`
    : `<tr><td>${r.submittedAt||''}</td><td>${optionLabel('menu_preencher',r.module)||r.module}</td><td>${cleanHtml(r.estado_aprovacao||'')}</td><td>${r.uuid||''}</td></tr>`
  ).join('') : `<tr><td colspan="${showChurch?5:4}">Sem registos recentes.</td></tr>`;
}
...
function renderFinancialReport(report){
  ...
  const scopeLabel=report.scope?.label || getEffectiveWorkChurch()?.name || (isDistrictUserClient()?'Visão distrital — todas as igrejas':'Igreja atribuída');
  const warnings=(report.warnings||[]).map(x=>`<div class="report-warning">${cleanHtml(x)}</div>`).join('');

  c.innerHTML = `
    <h2 class="report-title">Relatório Financeiro</h2>
    <div class="report-meta">
      <span>Âmbito: <strong>${cleanHtml(scopeLabel)}</strong></span>
...
async function loadFinancialReport(){
  ...
  const token = getAuthToken();
  if(!token){ showLogin('Sessão expirada. Faça login novamente.'); return; }
  const params = new URLSearchParams({ action:'report', token, ...currentReadScopeParams_() });
  ...
  try {
    ...
    console.error(e);
    $('#reportContainer').innerHTML = `<div class="empty-report">Não foi possível carregar o relatório: ${cleanHtml(e.message||'erro desconhecido')}.</div>`;
...
function populateApprovalFilters(){
  ...
  if(church){
    const previous=church.value;
    const workingId=getSelectedWorkChurchId();
    const list=isDistrictUserClient()
      ? (state.churches||[])
      : [getEffectiveWorkChurch()].filter(Boolean).map(x=>({igreja_id:x.id,nome_igreja:x.name}));
    church.innerHTML='<option value="">Todas as igrejas permitidas</option>'+list.map(c=>{
      const id=String(c.igreja_id||c.id||'');
      const name=String(c.nome_igreja||c.label||c.name||id);
      return `<option value="${approvalEscape(id)}">${approvalEscape(name)}</option>`;
    }).join('');
    const preferred=previous || (isDistrictUserClient()?workingId:'');
    if(preferred && list.some(c=>String(c.igreja_id||c.id||'')===preferred)) church.value=preferred;
  }
  if(module){
...
async function loadAppData(){
  ...
  if(!url || !token) return;
  try{
    const params=new URLSearchParams({action:'appdata',token,...currentReadScopeParams_()});
    const res = await fetch(url + '?' + params.toString(),{cache:'no-store'});
    const out = await res.json();
    ...
    console.warn(e);
    toast(e.message||'Não foi possível carregar os dados do módulo.','error');
  }
}
...
function avanteVariation(base,current){
  if(base===null||base===undefined||current===null||current===undefined) return {comparable:false,delta:null,growthPct:null};
  const b=Number(base),c=Number(current);
  return {comparable:true,delta:c-b,growthPct:b!==0?(c-b)/b*100:null};
}
function avanteSignedMoney(value){
  const n=Number(value||0);
  return `${n>0?'+':''}${money(n)}`;
}
function avantePercent(value){
...
function populateAvanteComparisonSelectors(){
  ...
  const baseSel=$('#avanteBaseYear'), compareSel=$('#avanteCompareYear'), churchSel=$('#avanteTrendChurch');
  if(!baseSel||!compareSel||!churchSel) return;
  const current=Number(comp.currentYear||years[years.length-1]);
  const previous=years.filter(y=>y<current).slice(-1)[0] || years[0];
  const base=Number(state.avante.comparisonBaseYear||previous);
  const compare=Number(state.avante.comparisonYear||current);
  const options=years.map(y=>`<option value="${y}">${y}${y===current&&comp.provisional?' — em curso':''}</option>`).join('');
  baseSel.innerHTML=options;
  compareSel.innerHTML=options;
  baseSel.value=years.includes(base)?String(base):String(previous);
  ...
  state.avante.trendChurchId=churchSel.value;
}
function renderAvanteAnnualBars(){
  const comp=avanteComparison(), el=$('#avanteAnnualBars'); if(!comp||!el) return;
  const rows=comp.years||[], max=Math.max(1,...rows.map(x=>Number(x.total||0)));
  el.innerHTML=rows.map(x=>{
    const width=Math.max(3,Number(x.total||0)/max*100);
    const provisional=String(x.status).toUpperCase()==='PROVISORIO';
    return `<div class="avante-annual-row"><div class="avante-annual-label"><strong>${x.year}</strong><small>${provisional?'Provisório':'Final'} · ${x.churchesWithData} igrejas com dados</small></div><div class="avante-annual-track"><i style="width:${width}%"></i></div><strong class="avante-annual-value">${money(x.total)}</strong></div>`;
  }).join('');
}
function renderAvanteChurchTrend(){
  ...
  el.innerHTML=years.map((y,i)=>{
    const v=vals[i];
    if(v===null) return `<div class="avante-annual-row"><div class="avante-annual-label"><strong>${y}</strong><small>Sem dados</small></div><div class="avante-annual-track empty"></div><strong class="avante-annual-value">—</strong></div>`;
    const width=Math.max(3,Number(v||0)/max*100);
    return `<div class="avante-annual-row"><div class="avante-annual-label"><strong>${y}</strong><small>${y===comp.currentYear&&comp.provisional?'Provisório':'Total anual'}</small></div><div class="avante-annual-track"><i style="width:${width}%"></i></div><strong class="avante-annual-value">${money(v)}</strong></div>`;
  }).join('');
}
function renderAvanteComparison(){
  const comp=avanteComparison(); if(!comp) return;
  populateAvanteComparisonSelectors();
  const baseYear=Number(state.avante.comparisonBaseYear), compareYear=Number(state.avante.comparisonYear);
  const baseSummary=(comp.years||[]).find(x=>Number(x.year)===baseYear);
  const compareSummary=(comp.years||[]).find(x=>Number(x.year)===compareYear);
  const variation=avanteVariation(baseSummary?.total,compareSummary?.total);
  renderKpis('avanteComparisonKpis',[
    {label:`Total ${baseYear}`,value:baseSummary?money(baseSummary.total):'Sem dados'},
    {label:`Total ${compareYear}`,value:compareSummary?money(compareSummary.total):'Sem dados'},
    {label:'Variação absoluta',value:variation.comparable?avanteSignedMoney(variation.delta):'—'},
    {label:'Crescimento',value:variation.comparable?avantePercent(variation.growthPct):'—'},
    {label:`Cobertura ${compareYear}`,value:compareSummary?`${compareSummary.churchesWithData} igrejas`:'—'}
  ]);
  const notice=$('#avanteComparisonNotice');
  if(notice){
    const currentIncluded=compareYear===Number(comp.currentYear)&&comp.provisional;
    notice.className='avante-rule-note'+(currentIncluded?'':' ok');
    notice.textContent=currentIncluded?comp.note:`Comparação de totais anuais entre ${baseYear} e ${compareYear}.`;
  }
  const status=$('#avanteComparisonStatus');
  if(status) status.textContent=compareYear===Number(comp.currentYear)&&comp.provisional?`${compareYear} · Provisório`:`${baseYear} → ${compareYear}`;

  const groupBody=$('#avanteGroupComparisonTable tbody');
  if(groupBody){
    groupBody.innerHTML=(comp.groups||[]).map(g=>{
      const base=avanteYearValue(g.values,baseYear),current=avanteYearValue(g.values,compareYear),v=avanteVariation(base,current);
      const cls=v.comparable?(v.delta>0?'positive':(v.delta<0?'negative':'neutral')):'neutral';
      return `<tr><td><strong>${avanteEscape(g.name)}</strong></td><td>${base===null?'—':money(base)}</td><td>${current===null?'—':money(current)}</td><td class="${cls}">${v.comparable?avanteSignedMoney(v.delta):'—'}</td><td class="${cls}">${v.comparable?avantePercent(v.growthPct):'—'}</td></tr>`;
    }).join('');
  }
...
function bindAvanteModule(){
  ...
  $('#avanteBaseYear')?.addEventListener('change',()=>{state.avante.comparisonBaseYear=Number($('#avanteBaseYear').value);renderAvanteComparison();});
  $('#avanteCompareYear')?.addEventListener('change',()=>{state.avante.comparisonYear=Number($('#avanteCompareYear').value);renderAvanteComparison();});
  $('#avanteTrendChurch')?.addEventListener('change',()=>{state.avante.trendChurchId=$('#avanteTrendChurch').value;renderAvanteChurchTrend();});
...
// v54.5.0 — isolamento por igreja, escopo distrital/local e permissões reforçadas.

...
// v47 — Normalização definitiva dos grupos/igrejas das Contribuições; anti-cache
...
function avanteCanonicalGroupId(value){
  ...
  const m=raw.match(/(?:grupo|grp|g)?\s*0*([1-8])/);
...
function avanteNormalizarData(data){
  ...
  let merged=incoming.length?incoming.map((g,gi)=>{
    ...
    const gname=String(g.name ?? g.grupo_nome ?? (lg?lg.name:''));
    const sourceChurches=(Array.isArray(g.churches)&&g.churches.length) ? g.churches : (lg?lg.churches:[]);
    const normalChurches=sourceChurches.map((c,i)=>{
      const lc=(lg&&lg.churches&&lg.churches[i])||{};
      const meta=Number(c.meta ?? c.meta_individual ?? c.metaIndividual ?? lc.meta ?? g.metaIndividual ?? g.meta_individual ?? (lg?lg.metaIndividual:0));
      const primeira=Number(c.primeira ?? c.primeiraTotal ?? c.primeira_contribuicao ?? 0);
      const segunda=Number(c.segunda ?? c.segundaTotal ?? c.segunda_contribuicao ?? 0);
      return {
        id:String(c.id ?? c.igreja_id ?? lc.id ?? (gid+'_'+(i+1))),
        name:String(c.name ?? c.igreja_nome ?? c.nome_igreja ?? lc.name ?? ''),
        meta,primeira,segunda,total:Number(c.total ?? (primeira+segunda)),
        segundaAberta: Boolean(c.segundaAberta ?? c.segunda_aberta ?? (primeira>=meta)),
    ...
    return {
      ...
      igrejasSegundaAberta:Number(g.igrejasSegundaAberta ?? g.igrejas_segunda_aberta ?? normalChurches.filter(c=>c.segundaAberta).length),
      churches:normalChurches.length ? normalChurches : (lg?lg.churches:[])
    };
  }):local.groups;
  const byId=new Map();
  merged.forEach(g=>{ if(g&&g.id&&!byId.has(g.id)) byId.set(g.id,g); });
  local.groups.forEach(g=>{
    if(!byId.has(g.id)) byId.set(g.id,g);
    else if(!byId.get(g.id).churches || !byId.get(g.id).churches.length) byId.get(g.id).churches=g.churches;
  });

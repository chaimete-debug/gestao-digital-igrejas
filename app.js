...
// v46 — Consolidação: corrige igrejas vazias no módulo Contribuições; anti-cache
...
function avanteNormalizarData(data){
  ...
  const localById=new Map(local.groups.map(g=>[String(g.id),g]));
  const incoming=(data&&Array.isArray(data.groups))?data.groups:[];
  const merged=incoming.length?incoming.map(g=>{
    const lg=localById.get(String(g.id));
    const churches=Array.isArray(g.churches)&&g.churches.length ? g.churches : (lg?lg.churches:[]);
    const normalChurches=churches.map((c,i)=>{
      const lc=(lg&&lg.churches&&lg.churches[i])||{};
      const meta=Number(c.meta ?? c.meta_individual ?? lc.meta ?? g.metaIndividual ?? g.meta_individual ?? 0);
      const primeira=Number(c.primeira ?? c.primeiraTotal ?? 0);
      const segunda=Number(c.segunda ?? c.segundaTotal ?? 0);
      return {
        id:String(c.id ?? c.igreja_id ?? lc.id ?? (String(g.id)+'_'+(i+1))),
        name:String(c.name ?? c.igreja_nome ?? lc.name ?? ''),
        meta,primeira,segunda,total:Number(c.total ?? (primeira+segunda)),
        segundaAberta: Boolean(c.segundaAberta ?? (primeira>=meta)),
        canEdit: c.canEdit!==undefined ? c.canEdit : true
      };
    }).filter(c=>c.id&&c.name);
    const primeiraTotal=Number(g.primeiraTotal ?? normalChurches.reduce((a,c)=>a+Number(c.primeira||0),0));
    const segundaTotal=Number(g.segundaTotal ?? normalChurches.reduce((a,c)=>a+Number(c.segunda||0),0));
    const metaGrupo=Number(g.metaGrupo ?? g.meta_grupo ?? (lg?lg.metaGrupo:0));
    return {
      id:String(g.id ?? g.grupo_id),
      name:String(g.name ?? g.grupo_nome ?? (lg?lg.name:'')),
      metaIndividual:Number(g.metaIndividual ?? g.meta_individual ?? (lg?lg.metaIndividual:0)),
      metaGrupo,primeiraTotal,segundaTotal,total:Number(g.total ?? (primeiraTotal+segundaTotal)),
      igrejasSegundaAberta:Number(g.igrejasSegundaAberta ?? normalChurches.filter(c=>c.segundaAberta).length),
      churches:normalChurches
    };
  }):local.groups;
  const existing=new Set(merged.map(g=>String(g.id)));
  local.groups.forEach(g=>{ if(!existing.has(String(g.id))) merged.push(g); });
...
function renderAvanteModule(){
  ...
  selects.forEach(sel=>{
    const current=sel.value||group.id;
    sel.innerHTML='';
    avanteState.groups.forEach(g=>sel.add(new Option(g.name,g.id)));
    sel.value=avanteState.groups.some(g=>g.id===current)?current:group.id;
  });
  ...
  const churchSel=$('#avanteChurchSelect'); if(churchSel){
    ...
    const churches=group.churches||[];
    churchSel.innerHTML=churches.length
      ? churches.map(c=>`<option value="${escapeHtml(c.id)}">${escapeHtml(c.name)}</option>`).join('')
      : '<option value="">Sem igrejas neste grupo</option>';

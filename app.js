...
function applyPermissionsToUi(){
  ...
  $$('.nav-btn').forEach(btn => {
    ...
    if(btn.dataset.districtOnly === 'true') {
      const district = !!u.isDistrict || String(u.igreja_id || u.churchId || '').trim() === '*' || String(u.nivel_acesso || u.accessLevel || '').toUpperCase() === 'DISTRITAL';
...
function calculateMainField(field){
  ...
  switch(name){
    ...
    case 'departamento_final':
      return getVal('tipo')==='Entrada' && getVal('rubrica')==='DIZ' ? 'ADMIN' : getVal('departamento');
    case 'departamento_final_label': return optionLabel('departamentos', getVal('departamento_final'));
    case 'valor_mzn': {
...
function validateCrossFieldRules(){
  ...
  if(module==='registo_financeiro'){
    ...
    if(!getVal('departamento_final')){
      markError(visibleInputByField('departamento'),'Seleccione o departamento/ministério.');
      ok=false;
    }
...
async function bootstrap(){
  ...
  try{
    ...
    // Falhas de rede, CORS, JSON ou indisponibilidade temporária não devem
    // apagar uma sessão local válida.
    const storedUser = getStoredUser();
    if(storedUser){
      state.currentUser = storedUser;
      showApp(storedUser);
      setStatus('bad','Ligação temporariamente indisponível','A sessão foi mantida. Tente actualizar os dados.');
      toast('A sessão foi mantida, mas não foi possível confirmar o backend.', 'error');
      return;
    }
    showLogin(message);
  }
...
// v52 — correcção de sessão persistente.
// Não limpar a sessão no arranque. A versão anterior executava clearAuthSession()
// sempre que a página carregava; por isso o login era aceite, mas a aplicação
// voltava imediatamente para o ecrã de login.
(function startAuthenticatedApp(){
  ...
  if(token && user && Object.keys(user).length){
    state.currentUser = user;
    showApp(user);
    setStatus('ok','A retomar sessão','A confirmar a sessão no backend...');
    bootstrap();
    return;
  }

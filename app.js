...
// v43 — Avante Evangelho 2026 + correcção consolidada dos ITs no backend
...
async function loadAvanteData(){
  ...
  try{
    ...
    const normalizar=(data)=>{
      ...
      return {ok:!!(data&&data.ok),groups};
    };
    if(SANDBOX_MODE){
      out=avanteSandboxData();
    }else{
      ...
      try{
        ...
        out=normalizar(await res.json());
        if(!out.ok) throw new Error((out&&out.message)||'Não foi possível carregar as contribuições.');
      }catch(authErr){
        console.warn('Falha no endpoint autenticado das contribuições; a tentar endpoint público.', authErr);
        try{
          const resPublic=await fetch(url+'?'+new URLSearchParams({action:'avantepublicdata'}).toString(),{cache:'no-store'});
          const pub=normalizar(await resPublic.json());
          if(pub.ok && pub.groups && pub.groups.length){
            out=pub;
            const stamp=$('#avanteDataNote');
            if(stamp) stamp.textContent='Dados carregados pela leitura pública. O registo continua sujeito às permissões do utilizador.';
          }else{
            throw new Error((pub&&pub.message)||'Endpoint público sem grupos.');
          }
        }catch(publicErr){
          console.warn('Falha no endpoint público das contribuições; a usar dados locais temporários.', publicErr);
          out=avanteSandboxData();
          const stamp=$('#avanteDataNote');
          if(stamp) stamp.textContent='Dados locais temporários carregados. Para gravar contribuições, confirme se o backend v43 foi publicado e se executarPreparacaoV42() já foi executada.';
        }
      }
    }
    avanteState.groups=(out&&out.groups)||[];
    if(!avanteState.groups.length){
      if(body) body.innerHTML='<tr><td colspan="7">Não há grupos carregados. Execute a função executarPreparacaoV42() no Apps Script e publique novamente o backend.</td></tr>';
      const k=$('#avanteKpis'); if(k) k.innerHTML='<div class="avante-empty">Não há grupos carregados. Verifique as folhas Avante_Grupos e Avante_Igrejas.</div>';
      ['#avanteGroupSelect','#avanteFormGroup','#avanteChurchSelect'].forEach(sel=>{const el=$(sel); if(el) el.innerHTML='<option value="">Sem dados</option>';});
      return;
    }
...
function renderAvanteModule(){
  ...
  selects.forEach(s=>{s.innerHTML=avanteState.groups.map(g=>`<option value="${escapeHtml(g.id)}">${escapeHtml(g.name)}</option>`).join('');s.value=group.id;});

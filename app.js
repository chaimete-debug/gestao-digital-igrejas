...
// v54.2.2: mantém a autenticação por separador; hotfix visual da descrição lateral. Ao fechar o
...
function setCurrentUserCard(user){
  ...
  if(churchEl) churchEl.textContent = u.churchName || u.igreja_nome || u.igreja_id || 'Igreja';
  updateChurchScopeUi();
}
...
function setSelectedWorkChurch(id){
  const key=String(id||'').trim();
...
function updateChurchScopeUi(){
  ...
  const churchEl=$('#currentUserChurch');
  if(churchEl){
    if(district) churchEl.textContent=church ? church.name : 'Seleccione uma igreja';
    else churchEl.textContent=church?.name || currentUser().igreja_nome || currentUser().churchName || currentUser().igreja_id || 'Igreja não atribuída';
  }
...
if(workChurchSelect && workChurchSelect.dataset.bound!=='1'){
  ...
  workChurchSelect.addEventListener('change',()=>setSelectedWorkChurch(workChurchSelect.value));
}
...
// v54.2.2 — sessão segura por separador; hotfix visual.
